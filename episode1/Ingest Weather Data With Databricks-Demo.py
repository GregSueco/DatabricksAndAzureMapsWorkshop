...
# MAGIC %fs ls /tmp/ingestGettingStartedDemo/
...
# MAGIC %sql
# MAGIC CREATE DATABASE IF NOT EXISTS ingest_bronze LOCATION 'dbfs:/tmp/copy_into/copyfolder/user';
...
# MAGIC %sql
# MAGIC COPY INTO delta.`dbfs:/tmp/copy_into/copyfolder/user`   FROM 'dbfs:/tmp/ingestGettingStartedDemo/'   FILEFORMAT = CSV  FORMAT_OPTIONS('header'='true');

# COMMAND ----------

# DBTITLE 1,View the Delta Files
# MAGIC %fs ls dbfs:/tmp/copy_into/copyfolder/user

# COMMAND ----------

# DBTITLE 1,Create a Table "weather"
# MAGIC %sql
# MAGIC CREATE TABLE weather USING DELTA LOCATION 'dbfs:/tmp/copy_into/copyfolder/user';
...
# MAGIC %fs ls dbfs:/tmp/ingestAutoloader/autoloader/
